...
export function MessageReactions({ messageId }: MessageReactionsProps) {
  ...
  useEffect(() => {
    if (lastJsonMessage?.type === 'reaction_update' && lastJsonMessage.data.messageId.toString() === messageId) {
      const newReactions = new Map<string, ReactionState>();
      
      // Group reactions by emoji
      lastJsonMessage.data.reactions.forEach((reaction: {
        userId: number;
        username: string;
        displayName?: string;
        emoji: string;
